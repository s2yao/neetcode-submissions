...
class Solution:
    def countComponents(self, n: int, edges: List[List[int]]) -> int:
        uf = UnionFind(n)
        num_of_redundant_edge = 0
        # construct unionfind
        for node1, node2 in edges:
            if not uf.union(node1, node2):
                num_of_redundant_edge += 1

        ret = n + num_of_redundant_edge

        # check if 2 edges have same root
        for node1, node2 in edges:
            # need a way to find out if current edge is redundant
